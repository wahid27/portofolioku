...
import { motion, AnimatePresence, useInView, animate, useMotionValue, useTransform, useSpring } from 'framer-motion';
import { 
  Terminal, Cpu, BookOpen, Zap, X, Monitor, 
  Globe, Mail, Phone, ExternalLink, ArrowUpRight, 
  Code2, Rocket, ShieldCheck, Users, Activity,
  Instagram, Share2, Radio, Link2, Scan, MessageSquare, ShieldAlert
} from 'lucide-react';
...
const PROJECTS = [
  {
    ...
    problem: "Guru terjebak administrasi, bukan di depan siswa.",
  ...
  },
  {
    ...
    problem: "Rumus fisika sulit dibayangkan oleh siswa.",
    impact: "Meningkatkan pemahaman visual sebesar 80%.",
  ...
  },
  {
    ...
    problem: "Miskomunikasi requirement menghancurkan proyek.",
  ...
  },
  {
    ...
    problem: "UMKM lokal terlihat tidak profesional secara digital.",
  ...
  },
  {
    ...
    type: "Future Concept",
    shortDesc: "Eksperimen interaksi web modern yang menembus batas browser.",
    problem: "Website standar saat ini terlalu kaku dan membosankan.",
    impact: "Mendefinisikan ulang standar interaksi digital.",
  ...
  },
  {
    ...
    problem: "Identitas digital korporasi yang kurang otoritas.",
    impact: "Kehadiran digital yang otoriter dan stabil.",
    tech: ["Security", "SEO"],
  ...
  <div className="fixed right-0 top-1/2 -translate-y-1/2 z-[100] hidden lg:flex flex-col items-center gap-10 pr-6">
    <div className="flex flex-col gap-8">
      <motion.a href="https://www.tiktok.com/@nugas.id99" target="_blank" whileHover={{ x: -10, scale: 1.1 }} className="group relative flex items-center justify-end">
        <span className="absolute right-12 text-[10px] font-black uppercase tracking-[0.3em] opacity-0 group-hover:opacity-100 transition-all text-white bg-black px-3 py-1 border border-white/20 whitespace-nowrap">SIGNAL_A: TIKTOK</span>
      ...
      <motion.a href="https://www.instagram.com/wachid_amiruddin" target="_blank" whileHover={{ x: -10, scale: 1.1 }} className="group relative flex items-center justify-end">
        <span className="absolute right-12 text-[10px] font-black uppercase tracking-[0.3em] opacity-0 group-hover:opacity-100 transition-all text-white bg-black px-3 py-1 border border-white/20 whitespace-nowrap">SIGNAL_B: INSTAGRAM</span>
    ...
    <div className="rotate-90 text-[8px] font-black tracking-[0.8em] uppercase opacity-20 whitespace-nowrap">EXT_BROADCAST_ACTIVE</div>
  </div>
);

/** * ============================================================================
 * 3. 3D VISUALIZER ENGINE
 * ============================================================================
 */
const AdvancedNexusVisualizer = ({ color }) => {
  ...
  return (
    <div ref={containerRef} onMouseMove={handleMouseMove} onMouseLeave={() => { mouseX.set(0); mouseY.set(0); }} className="relative w-full h-full flex items-center justify-center overflow-hidden bg-neutral-950/40 border border-white/10" style={{ perspective: "1000px" }}>
      <motion.div animate={{ top: ['-10%', '110%'] }} transition={{ repeat: Infinity, duration: 4, ease: "linear" }} className="absolute left-0 right-0 h-[2px] z-50 pointer-events-none" style={{ background: `linear-gradient(90deg, transparent, ${color}, transparent)`, boxShadow: `0 0 20px ${color}` }} />
...
export default function App() {
  ...
  useEffect(() => {
    let logIdx = 0;
    const interval = setInterval(() => {
      if (logIdx < bootLogs.length) { setLoadingText(bootLogs[logIdx]); logIdx++; } 
      else { setLoading(false); clearInterval(interval); }
    }, 600);
    const handleMouseMove = (e) => setMousePos({ x: e.clientX, y: e.clientY });
    const cycleHeadline = setInterval(() => setHeadlineIndex(prev => (prev + 1) % HEADLINES.length), 6000);
    window.addEventListener('mousemove', handleMouseMove);
    return () => { clearInterval(interval); clearInterval(cycleHeadline); window.removeEventListener('mousemove', handleMouseMove); };
  }, []);
        ...
        <div className="max-w-md w-full relative">
          ...
          <div className="bg-neutral-900 border-l-2 border-[#00ff41] p-6 space-y-4 shadow-[0_0_50px_rgba(0,255,65,0.1)]">
             <div className="flex items-center gap-3">
                ...
                <AnimatePresence mode="wait"><motion.div key={loadingText} initial={{ opacity: 0, x: -5 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 5 }} className="font-bold text-sm uppercase">{">"} {loadingText}</motion.div></AnimatePresence>
             </div>
             <div className="flex gap-1 h-1">
                {Array.from({ length: 20 }).map((_, i) => (<motion.div key={i} animate={{ opacity: [0.1, 1, 0.1] }} transition={{ repeat: Infinity, duration: 1, delay: i * 0.05 }} className="flex-grow bg-[#00ff41]" />))}
             </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-black text-white font-mono selection:bg-[#ff0055] overflow-x-hidden">
      ...
      <main className="relative pt-40 px-6 max-w-7xl mx-auto pb-20">
        ...
        <section className="mb-48 grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
           <div>
              ...
              <div className="flex gap-2">
                 <button onClick={() => window.open(`https://wa.me/${whatsappNumber}`)} className="px-8 py-4 bg-white text-black text-[10px] font-black uppercase tracking-[0.3em] hover:bg-[#00ff41] transition-all shadow-[0_0_30px_rgba(255,255,255,0.1)]">Start_Evolve</button>
           ...
           {/* 2. THE MASTER VISUALIZER (3D SCANNING) */}
        ...
        <section className="mb-40 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
           {[
             ...
             <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="border border-white/5 p-8 bg-neutral-950 flex flex-col items-center group">
               <div style={{ color: m.color }} className="mb-4 opacity-50 group-hover:opacity-100 transition-opacity">{m.icon}</div>
               <div className="text-4xl font-black italic tracking-tighter mb-1">{m.val}</div>
               <div className="text-[10px] uppercase font-bold text-white/30 tracking-[0.3em]">{m.label}</div>
             </motion.div>
           ))}
        </section>
        ...
        <section className="mb-40">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
              {PROJECTS.map((p) => (
                <motion.div key={p.id} layoutId={`p-${p.id}`} onClick={() => setActiveProject(p)} whileHover={{ scale: 1.02 }} className="cursor-pointer bg-neutral-900/30 border border-white/5 p-10 relative overflow-hidden group transition-colors hover:border-white/20">
                  <div style={{ color: p.color }} className="text-[9px] font-black uppercase tracking-[0.4em] mb-4">Task_Report_0{p.id}</div>
                  <h4 className="text-3xl font-black uppercase italic leading-none mb-6 group-hover:text-white transition-colors">{p.title}</h4>
                  ...
                  <div className="flex items-center justify-between border-t border-white/5 pt-6">
                    ...
                    <div className="text-[10px] font-black text-[#00ff41] flex items-center gap-2 underline uppercase">View_Case</div>
                  </div>
                </motion.div>
              ))}
           </div>
        ...
        <footer className="relative mt-40">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-px bg-white/10 border border-white/10 mb-20 overflow-hidden shadow-[0_0_100px_rgba(255,255,255,0.05)]">
                ...
                <motion.a 
                    ...
                    target="_blank"
                    className="relative p-12 md:p-20 bg-black hover:bg-[#00ff41] hover:text-black transition-all group overflow-hidden"
                >
                    <div className="relative z-10">
                        <div className="flex items-center gap-4 mb-10">
                            <div className="w-12 h-12 border-2 border-current flex items-center justify-center"><MessageSquare size={24} /></div>
                            <span className="text-[10px] font-black uppercase tracking-[0.5em]">Command_01: WhatsApp</span>
                        </div>
                        <h2 className="text-5xl md:text-7xl font-black italic uppercase leading-none mb-8 tracking-tighter">SIAP_ <br/>EKSEKUSI.</h2>
                        <div className="flex items-center gap-3 text-xs font-bold uppercase tracking-widest opacity-60 group-hover:opacity-100">
                           <span className="animate-pulse">●</span> ONLINE_NOW // TAP_TO_UPLINK
                        </div>
                    </div>
                ...
                <motion.a 
                    ...
                    className="relative p-12 md:p-20 bg-black hover:bg-[#ff0055] hover:text-white transition-all group overflow-hidden"
                >
                    <div className="relative z-10">
                        <div className="flex items-center gap-4 mb-10">
                            <div className="w-12 h-12 border-2 border-current flex items-center justify-center"><Mail size={24} /></div>
                            <span className="text-[10px] font-black uppercase tracking-[0.5em]">Command_02: Email</span>
                        </div>
                        <h2 className="text-5xl md:text-7xl font-black italic uppercase leading-none mb-8 tracking-tighter">KIRIM_ <br/>BRIEF.</h2>
            ...
            <div className="w-full flex flex-col md:flex-row justify-between items-center gap-10 pt-20 pb-40 text-[9px] font-bold text-white/20 uppercase tracking-[0.4em]">
                <div className="flex flex-col gap-2">
                    <span>© WAHID_NEXUS_ENGINE_v6.2</span>
                    <div className="flex items-center gap-2">
                        <div className="w-1.5 h-1.5 bg-[#00ff41] rounded-full animate-pulse" />
                        SYSTEM_ID: AMIRUDDIN_MASTER_CORE
                    </div>
                </div>
                <h2 className="text-4xl md:text-6xl font-black italic opacity-5 cursor-default">SCALE_BEYOND</h2>
      ...
      <AnimatePresence>
        {activeProject && (
          <>
            ...
            <motion.div layoutId={`p-${activeProject.id}`} className="fixed inset-4 md:inset-10 lg:inset-20 bg-neutral-950 border border-white/10 z-[151] p-10 md:p-20 overflow-y-auto shadow-[0_0_100px_rgba(0,255,65,0.1)]">
               <button onClick={() => setActiveProject(null)} className="absolute top-10 right-10 hover:rotate-90 transition-transform"><X size={40} /></button>
               <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
                  <div className="space-y-12">
                     <div>
                        <div className="text-[#ff0055] font-black text-[10px] tracking-[0.6em] mb-4 uppercase">Mission_Summary</div>
                        <h2 className="text-6xl md:text-8xl font-black uppercase italic leading-none mb-6 tracking-tighter">{activeProject.title}</h2>
                     </div>
                     <div className="space-y-10">
                        <section><h3 className="text-[#00ff41] text-[10px] font-black uppercase mb-4 tracking-[0.4em] flex items-center gap-4"><span className="w-10 h-[1px] bg-[#00ff41]" /> Friction</h3><p className="text-3xl text-white/80 italic font-bold leading-tight">"{activeProject.problem}"</p></section>
                        <section><h3 className="text-[#ff8800] text-[10px] font-black uppercase mb-4 tracking-[0.4em] flex items-center gap-4"><span className="w-10 h-[1px] bg-[#ff8800]" /> Evolution</h3><p className="text-3xl text-white/80 italic font-bold leading-tight">"{activeProject.impact}"</p></section>
                     </div>
                  </div>
                  <div className="bg-neutral-900 border border-white/5 flex flex-col h-[500px]">
                      ...
                      <div className="flex-grow flex flex-col items-center justify-center p-10 text-center relative overflow-hidden">
                        <Activity size={100} className="text-white opacity-5 mb-10" />
                        <a href={activeProject.url} target="_blank" rel="noopener noreferrer" className="relative z-10 px-12 py-6 bg-white text-black font-black uppercase text-xs hover:bg-[#00ff41] transition-all shadow-2xl tracking-widest">Deploy_Live_Mission</a>
                      </div>
                  </div>
               </div>
            </motion.div>
