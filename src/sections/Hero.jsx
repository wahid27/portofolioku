...
// --- KOMPONEN MESIN 3D (Nexus Sentinel Engine) ---
      ...
      <motion.div style={{ rotateX, rotateY, transformStyle: "preserve-3d" }} className="relative flex items-center justify-center w-full h-full">
        ...
        {[0, 1, 2].map((i) => (
          <motion.div
            ...
            style={{ width: '100px', height: '100px', borderColor: color, translateZ: '-50px' }}
          />
        ))}

        {/* Inti Kode 3D */}
        <motion.div 
          className="relative z-20 p-10 border-2 bg-black/80 backdrop-blur-sm"
          style={{ borderColor: color, boxShadow: `0 0 60px ${color}33`, translateZ: '150px' }}
        >
          <Code2 size={100} style={{ color: color }} strokeWidth={1.5} />
        </motion.div>

        {/* Data Telemetry (Teks Kecil di Sudut) */}
        <div className="absolute inset-0 p-8 font-mono text-[7px] uppercase tracking-[0.4em] opacity-40 flex flex-col justify-between pointer-events-none" style={{ color: color, translateZ: '200px' }}>
          <div className="flex justify-between">
            ...
            <span>FREQ_OSC: 80.56 MHz</span>
          </div>
          <div className="flex justify-between items-end">
            <div className="flex flex-col gap-1">
              <span>CORE_TEMP: OPTIMAL</span>
              <span>LOGIC_GATES: OPEN</span>
            </div>
            <span>WAHID_NEXUS_v6.2</span>
          </div>
        </div>
      </motion.div>
    </div>
  ...
  <section className="mb-48 grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
    <div>
      ...
      <div className="h-[120px] border-l-2 border-white/10 pl-8 mb-10">
        <AnimatePresence mode="wait">
          <motion.p key={`sub-${headlineIndex}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-lg text-white/50 italic leading-relaxed font-bold">
    ...
    {/* BAGIAN VISUALIZER YANG KITA AKTIFKAN */}
